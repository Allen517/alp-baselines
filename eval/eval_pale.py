...
class Eval_PALE(Eval):
    ...
    def calc_mrr_by_dist(self, **kwargs):
        ...
        with open(kwargs['out_file'], 'w') as fout:
            cnt = 0
            wrt_lns = ''
            to_keys = list(self.inputs['end'].keys())
            to_size = len(to_keys)
            ...
            for nd_from, nds_to in self.labels['src2end'].items():
                for nd_to in nds_to:
                    ...
                    model_res = {
                        'from': self._calc_model_res(inputs=[self.inputs['src'][nd_from]]
                                                        , n_layer=kwargs['n_layer']),
                        'to': self.inputs['end'][nd_to],
                        'rand': None
                    }
                    ...
                    for k in range(kwargs['candidate_num']):
                        ...
                        model_res['rand'] = self.inputs['end'][rand_nd_to]
